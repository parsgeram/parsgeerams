...
import org.pouyadr.ui.ProfileActivity;
...
public class MessagesController implements NotificationCenter.NotificationCenterDelegate {
    ...
    public ArrayList<TLRPC.Dialog> dialogsfaver=new ArrayList<>();
    public ArrayList<TLRPC.Dialog> dialogsJustGroupsOnly=new ArrayList<>();
    public ArrayList<TLRPC.Dialog> dialogsBotOnly=new ArrayList<>();
    public ArrayList<TLRPC.Dialog> dialogsChannelOnly=new ArrayList<>();
    public ArrayList<TLRPC.Dialog> dialogsContactOnly=new ArrayList<>();
    public ArrayList<TLRPC.Dialog> dialogsCreatorOnly=new ArrayList<>();
    public ArrayList<TLRPC.Dialog> dialogsHiddenOnly=new ArrayList<>();
        ...
        if (UserConfig.isClientActivated()) {
            if (ConnectionsManager.getInstance().getPauseTime() == 0 && ApplicationLoader.isScreenOn && !ApplicationLoader.mainInterfacePaused) {
                if (statusSettingState != 1 && (lastStatusUpdateTime == 0 || Math.abs(System.currentTimeMillis() - lastStatusUpdateTime) >= 55000 || offlineSent)) {
                    ...
                    req.offline = Setting.getGhostMode();
                    statusRequest = ConnectionsManager.getInstance().sendRequest(req, new RequestDelegate() {
                        @Override
                        public void run(TLObject response, TLRPC.TL_error error) {
                            if (error == null) {
                                if(!Setting.getGhostMode()) lastStatusUpdateTime = System.currentTimeMillis();
                                offlineSent = Setting.getGhostMode();
                                statusSettingState = 0;
                            } else {
            ...
            public void run() {
                ...
                AndroidUtilities.runOnUIThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        dialogsunread.clear();
                        dialogsfaver.clear();
                        dialogsJustGroupsOnly.clear();
                        dialogsSuperGroupsOnly.clear();
                        dialogsChannelOnly.clear();
                        dialogsCreatorOnly.clear();
                        dialogsContactOnly.clear();
                        dialogsHiddenOnly.clear();
                        ...
                        for (int a = 0; a < dialogs.size(); a++) {
                            ...
                            ////////////////////////////////
                            TLRPC.Dialog dialog=d;
                            if (dialog instanceof TLRPC.TL_dialog) {
                                if (dialog.id < 0) {
                                    TLRPC.Chat chat = (TLRPC.Chat) chatsDict.get(Integer.valueOf(-((int) dialog.id)));
                                    if (!migrate || chat == null || chat.migrated_to == null) {
                                        MessagesController.this.dialogsGroupsOnly.add(dialog);

                                        MessagesController.this.dialogsJustGroupsOnly.add(dialog);
                                        //setting
                                        if (chat != null && (chat.creator || (true && chat.editor))) {
                                            dialogsCreatorOnly.add(dialog);
                                        }
                                    } else {
                                        dialogs.remove(a);
                                        a--;
                                    ...
                                    TLRPC.User user = MessagesController.this.getUser(Integer.valueOf((int) dialog.id));
                                    if (user == null || !user.bot) {
                                        MessagesController.this.dialogsContactOnly.add(dialog);
                                    } else {
                                        MessagesController.this.dialogsBotOnly.add(dialog);
                                    }
                                }
                            } else if (dialog instanceof TLRPC.TL_dialogChannel) {
                                TLRPC.Chat chat =getChat(Integer.valueOf(-((int) dialog.id)));
                                if (chat != null && ((chat.megagroup && chat.editor) || chat.creator)) {
                                    MessagesController.this.dialogsGroupsOnly.add(dialog);
                                }
                                //setting//
                                if (chat != null && (chat.creator || (true && chat.editor))) {
                                    MessagesController.this.dialogsCreatorOnly.add(dialog);
                                }
                                if (chat != null) {
                                    if (!ChatObject.isChannel(chat) || chat.megagroup) {
                                        MessagesController.this.dialogsSuperGroupsOnly.add(dialog);
                                    } else {
                                        MessagesController.this.dialogsChannelOnly.add(dialog);
                                    }
                                }
                            }

                            if (d.unread_count > 0) dialogsunread.add(d);
                            if (FavoriteController.isFavor(d.id))dialogsfaver.add(d);
                            if (HiddenController.isHidden(d.id))dialogsHiddenOnly.add(d);
                            int high_id = (int) (d.id >> 32);
                            final boolean isChat = d.id < 0 && high_id != 1;
                            TLRPC.User user = null;
                            if (!isChat && d.id > 0 && high_id != 1) {
                                user = MessagesController.getInstance().getUser((int) d.id);
                            }
    ...
    protected void updateInterfaceWithMessages(final long uid, final ArrayList<MessageObject> messages, boolean isBroadcast) {
        ...
        if (dialog == null) {
            if (!isBroadcast) {
                ...
                if (dialog instanceof TLRPC.TL_dialog) {
                    if (dialog.id < 0) {
                        chat = (TLRPC.Chat) chats.get(Integer.valueOf(-((int) dialog.id)));
                        if ( chat == null || chat.migrated_to == null) {
                            MessagesController.this.dialogsGroupsOnly.add(dialog);
                            MessagesController.this.dialogsJustGroupsOnly.add(dialog);
                            //setting
                   ...
                   chat =getChat(Integer.valueOf(-((int) dialog.id)));
                    if (chat != null && ((chat.megagroup && chat.editor) || chat.creator)) {
                        MessagesController.this.dialogsGroupsOnly.add(dialog);
                    }
                    //setting//
                ...
                ///////////////
                //DialogController.addDialog(dialog);
